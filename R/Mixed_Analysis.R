...
Mixed_Analysis <- function(input, rv) {
  ...
  if (type == 'Mix') {
    ...
    for (response in input$blue_resp) {
      ...
      base::sink(base::paste0('Summary of Model(', response, ').txt'))
      ...
      p <- ggplot2::ggplot(A, ggplot2::aes(x = Model@resp$y)) +
        ...
        ggplot2::xlab(base::paste0('Response(', response, ') Variable'))
      ...
      utils::write.csv(bb, paste0(input$project_name, ' -- ', response, ' -- Anova (random effect) Table.csv'))

      a <- stats::anova(MM.S)
      aa <- as.data.frame(a)

      utils::write.csv(aa, paste0(input$project_name, ' -- ', response, ' -- Anova table (fixed effect).csv'))

      MM.Sresid <- stats::residuals(MM.S, type = "pearson")
      MM.Sactual <- stats::predict(MM.S)
      utils::write.csv(MM.Sactual, paste0(input$project_name, ' -- ', response, ' -- Predicted Values.csv'), row.names = F)
      # create plot
      ...
      filesave('png', input$project_name, base::paste0(' -- ', response, ' -- Predicted vs Residual'), callback = p, rv)
      filesave('pdf', input$project_name, base::paste0(' -- ', response, ' -- Predicted vs Residual'), callback = p, rv)

      p <- function() {
        car::qqPlot(MM.Sresid)
      }

      filesave('png', input$project_name, base::paste0(' -- ', response, ' -- Quantile Normality'), callback = p, rv)
      ...
      utils::write.csv(buffer, paste0(input$project_name, ' -- ', response, ' -- Shapiro-Wilk normality test.csv'))
    }
  }
    ...
    for (response in input$blue_resp) {
      ...
      filesave('png', project_name, base::paste0(' -- ', response, ' -- Variance Proportion'), p1, rv)
      filesave('pdf', project_name, base::paste0(' -- ', response, ' -- Variance Proportion'), p1, rv)
    ...
    for (response in input$blue_resp) {
      ...
      p <- function() {
        base::plot(
          ...
          xlab = "Predicted data",
          ylab = "Residual data",
          main = base::paste0('Predicted vs Residual(', response, ')')
        )
      }
